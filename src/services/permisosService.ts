...
const getGruposPermisosService: string = `
    SELECT 
        g.id as grupo_id, g.nombre as grupo_nombre, 
        p.id as permiso_id, p.nombre as permiso_nombre, p.descripcion as permiso_descripcion 
    FROM grupos g
    LEFT JOIN grupos_permisos gp ON g.id = gp.grupo_id
...
const getUserWithPermissionsService: string = `
  SELECT 
    u.id, u.nombre, u.apellido, u.foto, u.identificador, u.id_proyecto,
    g.id as grupo_id, g.nombre as grupo_nombre,
    p.id as permiso_id, p.nombre as permiso_nombre, p.descripcion as permiso_descripcion
  FROM usuarios u
